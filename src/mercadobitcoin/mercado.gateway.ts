...
@WebSocketGateway()
export class MercadoGateway implements OnGatewayInit {
  private readonly logger: Logger = new Logger(MercadoGateway.name);
  ...
  subscribeToChannels() {
    this.tradingSymbols.forEach((symbol) => {
      ...
      const payload = {
        type: 'subscribe',
        subscription: { name: 'orderbook', id: tradingPair, limit: 10 },
      };
  ...
  afterInit(): void {
    ...
    this.clientSocket = new WebSocket('wss://ws.mercadobitcoin.net/ws');
  ...
  message(data) {
    ...
    if (message.event === 'subscribed') {
      ...
      if (payload !== 'hb') {
        const normalized = {
          exchange: 'MercadoBitcoin', // exchange
